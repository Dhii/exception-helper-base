...
use Dhii\Exception\CreateOutOfBoundsExceptionCapableTrait as TestSubject;
 ...
 * @since [*next-version*]
 */
class CreateOutOfBoundsExceptionCapableTraitTest extends TestCase
{
    /**
     ...
     * @since [*next-version*]
     */
    const TEST_SUBJECT_CLASSNAME = 'Dhii\Exception\CreateOutOfBoundsExceptionCapableTrait';
    ...
    public function testCanCreateInvalidArgumentException()
    {
        ...
        $result = $_subject->_createOutOfBoundsException($message, $code, $previous, $argument);

        $this->assertInstanceOf('OutOfBoundsException', $result, 'Subject could not create a valid result ');
        $this->assertInstanceOf('Dhii\Exception\OutOfBoundsExceptionInterface', $result, 'Subject could not create a valid result ');
